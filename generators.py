...
def test():
    ...
    print(
        list(data)
    )  # generator already finished next() stastments, and need to be reset, just run the above line again or store the data to have it again
...
def get_data_with_list_memory_efficiant():
    ...
    with open("dummy_data.txt", "r") as f:
        for index, line in enumerate(
            f, start=1
        ):  # start placeholder do iterate all list by act as a bias so start of 1 will start with index 1 instead of 0
            if line == "99000\n":
                data = index
                break
