...
def my_logger(orig_func):
    ...
    log = Logger(
        logFileName="test", loggerName=Path(sys.argv[0]).parts[-1], loggingLevel="DEBUG"
    )

    @wraps(
        orig_func
    )  # not mandatory here, added in the 'based on' video only when chaining(applying) two decorations on the same function to save original function name and idenity, probably best to try without this decorator first and see if works
    def wrapper(*args, **kwargs):
        log.logger.info("Ran with args: {}, and kwargs: {}".format(args, kwargs))
